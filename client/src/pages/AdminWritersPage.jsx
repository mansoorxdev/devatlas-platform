...
export function AdminWritersPage() {
  ...
  return (
    <>
      <Helmet>
        <title>Writer Management — DevAtlas Master</title>
      </Helmet>
        ...
        <Container className="max-w-6xl">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-200 dark:border-slate-800 mb-8">
            <div>
              ...
              <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">Writer Contributors</h1>
              <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
                View contributor accounts, review article contribution metrics, and manage active access status.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 mb-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400 font-medium">
              Total Writers Registered: <span className="font-bold text-slate-900 dark:text-slate-100">{pagination.total || 0}</span>
            </div>
          </div>
          ...
          {isLoading ? (
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-12 text-center shadow-sm">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Loading writers list...</p>
            </div>
          ) : error ? (
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-12 text-center shadow-sm">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-sm mx-auto">
                {searchQuery ? 'No writers match your search query.' : 'There are no writer accounts registered yet.'}
              </p>
            </div>
          ) : (
                ...
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-50 dark:bg-slate-950/80 border-b border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">
                    <tr>
                      ...
                      <th className="py-3.5 px-4">Article Metrics</th>
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 font-medium">
                    {writers.map((writer) => {
                      ...
                      const formattedJoined = new Date(writer.createdAt).toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric',
                      });
                      ...
                      return (
                        <tr key={writer.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                          <td className="py-3.5 px-4">
                            <div className="font-bold text-slate-900 dark:text-slate-100">{writer.name}</div>
                          ...
                          <td className="py-3.5 px-4 whitespace-nowrap">
                            <div className="flex items-center gap-3 text-[11px]">
                              <span className="text-slate-900 dark:text-slate-100 font-bold">{stats.total || 0} Total</span>
                              <span className="text-emerald-600 dark:text-emerald-400 font-semibold">{stats.published || 0} Published</span>
                              <span className="text-amber-600 dark:text-amber-400 font-semibold">{stats.pending_review || 0} Reviewing</span>
                              <span className="text-rose-600 dark:text-rose-400 font-semibold">{stats.changes_requested || 0} Action Needed</span>
                            </div>
                          </td>
                          ...
                          <td className="py-3.5 px-4 whitespace-nowrap text-right">
                            <button
                              ...
                              className={`px-3 py-1 rounded-xl text-xs font-semibold cursor-pointer inline-flex items-center gap-1 transition-colors ${
                                isActive
                                  ? 'bg-rose-100 dark:bg-rose-950 hover:bg-rose-200 text-rose-700 dark:text-rose-300'
                                  : 'bg-emerald-100 dark:bg-emerald-950 hover:bg-emerald-200 text-emerald-700 dark:text-emerald-300'
                              }`}
                            >
                              {isActive ? <UserX size={13} /> : <UserCheck size={13} />}
                              <span>{isActive ? 'Deactivate Account' : 'Reactivate Account'}</span>
                            </button>
                          </td>
                        </tr>
                      );
      ...
      {targetWriter && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-xl relative">
            ...
            <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-4 ${targetWriter.isActive ? 'bg-rose-500/10 text-rose-500' : 'bg-emerald-500/10 text-emerald-500'}`}>
            ...
            <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed mb-6">
              {targetWriter.isActive ? (
                <>
                  Are you sure you want to deactivate <span className="font-bold text-slate-900 dark:text-slate-100">{targetWriter.name}</span> ({targetWriter.email})? Deactivated writers cannot log in or create/submit articles. Existing articles remain intact.
                </>
              ) : (
                <>
